...
class LinearRegression(DiscriminativeClassifier):
	...
	def __init__(self, l2norm=1, preprocessing="z", eta=1e-4, max_epoch=30, l2_on=False, method="lsq"):
		"""
		:param l2norm: l2 norm penalty
		:param preprocessing: preprocessing method
		:param eta: learning rate (step size)
	...
	def test(self, eps=1e-4):
		...
		# Error rate
		ratio = -1
		self.weight = np.array([np.float(w) for w in self.weight])
		err = norm(self.weight, np.c_[np.ones(len(self.x_test)), self.x_test].T - self.y_test)
		for i in xrange(len(self.x_test)):
